...
$st2 = "سـتاد"." ".$club3;
  ...
  $ref2 = $input[$rand_keys[1]] . "\n";
     ...
     $sql1="SELECT * FROM `league`
      WHERE   
      ('$m1' = `match_1`) || ('$m1' = `match_2`) || ('$m1_2' = `match_1`) || ('$m1_2' = `match_2`) ||
      ('$m1_2' = `match_1`) || ('$m1_2' = `match_2`) || ('$m1_2' = `match_1`) || ('$m1_2' = `match_2`)
  
     ";
   
   $stmt_admin_info = $con->prepare("$sql1");
   $stmt_admin_info->execute(array());
     $ru_count_m2 = $stmt_admin_info->rowCount();

   if($ru_count_m2 == 0){ // إذا كان الناتج صفر يعني ان الاندية لم تلعب من قبل يتم إضافتها بطريقة عادية

 

    $sql="INSERT INTO `league` (`league_id`,`round_id`,`match_1`,`match_2`,`ref1`,`ref2`,`st1`,`st2`)
    VALUE (NULL , 3 ,'$m1','$m2','$ref1','$ref2','$st1','$st2')";
